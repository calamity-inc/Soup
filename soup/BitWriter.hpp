...
NAMESPACE_SOUP
{
	...
	public:
		...
		[[deprecated]] bool u16_dyn(uint16_t val); // A 16-bit value encoded using 9-17 bits. Assumes that smaller numbers are the norm.
		[[deprecated]] bool u16_dyn_2(uint16_t val); // A 16-bit value encoded using 5-18 bits. Assumes that smaller numbers are the norm.
		...
		[[deprecated]] bool str_utf32_lp(const std::u32string& str, uint8_t lpbits); // A UTF-32 string encoded using 6-23 bits per codepoint. Length-prefixed.
	private:
