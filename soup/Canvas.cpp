...
NAMESPACE_SOUP
{
	...
	bool Canvas::toBmp(Writer& w) const
	{
		uint16_t s;
		uint32_t i;
		SOUP_IF_UNLIKELY (!(s = 0x4D42, w.u16_le(s))
			|| !(i = static_cast<uint32_t>(40 + (3 * pixels.size())), w.u32_le(i))
			|| !w.skip(4)
			|| !(i = (14 + 40), w.u32_le(i))
			|| !(i = 40, w.u32_le(i))
			|| !(i = width, w.u32_le(i))
			|| !(i = height * -1, w.u32_le(i))
			|| !(s = 1, w.u16_le(s))
			|| !(s = 24, w.u16_le(s))
			|| !(i = 0, w.u32_le(i))
			|| !(i = static_cast<uint32_t>(3 * pixels.size()), w.u32_le(i))
			|| !w.skip(4 + 4 + 4 + 4)
			)
		{
			return false;
