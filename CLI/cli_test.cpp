...
#define TEST_CPU_X64(asm, ...) { uint8_t code[] = { __VA_ARGS__ }; const uint8_t* cp = code; assert(x64::disasm(cp).toString() == asm); assert(cp == &code[sizeof(code)]); }
		...
		TEST_CPU_X64("mov [rsp], rax", 0x48, 0x89, 0x04, 0x24);
		TEST_CPU_X64("mov [rsp+0x69], rbx", 0x48, 0x89, 0x5C, 0x24, 0x69);
		TEST_CPU_X64("mov [rax+0x69], rbx", 0x48, 0x89, 0x58, 0x69);

		TEST_CPU_X64("mov al, 69", 0xB0, 0x45);
		TEST_CPU_X64("mov ah, 69", 0xB4, 0x45);
		TEST_CPU_X64("mov ax, 69", 0x66, 0xB8, 0x45, 0x00);
		TEST_CPU_X64("mov eax, 69", 0xB8, 0x45, 0x00, 0x00, 0x00);
		TEST_CPU_X64("mov rax, 69", 0x48, 0xC7, 0xC0, 0x45, 0x00, 0x00, 0x00);
		TEST_CPU_X64("mov rax, 1384798605843239735", 0x48, 0xB8, 0x37, 0x13, 0xBE, 0xBA, 0xFE, 0xCA, 0x37, 0x13);

		TEST_CPU_X64("mov rbp, rbp", 0x48, 0x89, 0xED);
		TEST_CPU_X64("mov byte ptr [rip+0x69], 0", 0xC6, 0x05, 0x69, 0x00, 0x00, 0x00, 0x00);
		TEST_CPU_X64("mov byte ptr [rbp+0x69], 0", 0xC6, 0x45, 0x69, 0x00);

		TEST_CPU_X64("mov 0, 69", 0xC7, 0x04, 0x25, 0x00, 0x00, 0x00, 0x00, 0x45, 0x00, 0x00, 0x00);

		TEST_CPU_X64("ret", 0xC3);

		TEST_CPU_X64("push ax", 0x66, 0x50);
		TEST_CPU_X64("push rax", 0x50);
		TEST_CPU_X64("push rdi", 0x57);
		...
		TEST_CPU_X64("sub rsp, 32", 0x48, 0x83, 0xEC, 0x20);

		TEST_CPU_X64("cmp al, al", 0x38, 0xc0);
		TEST_CPU_X64("cmp rax, rax", 0x48, 0x39, 0xc0);
		TEST_CPU_X64("cmp al, 0", 0x3c, 0x00);
		TEST_CPU_X64("cmp rax, 0", 0x48, 0x83, 0xf8, 0x00);
		TEST_CPU_X64("cmp rax, 1762867049", 0x48, 0x3d, 0x69, 0x37, 0x13, 0x69);
		TEST_CPU_X64("cmp byte ptr [rax+0x69], 0", 0x80, 0x78, 0x69, 0x00);
		TEST_CPU_X64("cmp byte ptr [rax+0x69133769], 0", 0x80, 0xb8, 0x69, 0x37, 0x13, 0x69, 0x00);
		TEST_CPU_X64("cmp byte ptr [rip+0x69], 0", 0x80, 0x3d, 0x69, 0x00, 0x00, 0x00, 0x00);
	});
}
